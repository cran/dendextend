...
The function `match_order_by_labels` makes sure that the order in the leaves correspondes to the same labels in both trees.
...
The function `cor_cophenetic` is faster than `cor_bakers_gamma`, and might be prefered for that reason.
...
It is highly recommanded to install dendextendRcpp for regular use, and it is avilable on CRAN:
...
We can nicely visualize this using a dendrogram with the labels colored with the real species cateogry, and branches colored based on cutting the tree to 3 classes.
