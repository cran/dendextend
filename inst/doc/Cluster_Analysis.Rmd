...
In the heatmap we also see how the Setosa species has low Petal values (in light yellow), but it is very difficult to see any clear distinction in between the other two species.
...
We can see that the correlations are not so strong, indicating a behaviour which is dependent on some items which are very distant from one another having an influence on the pearson's correlation more than the spearman one.
...
We have 39 sub-trees which are identical between the two dendrograms:
...
Lastly, when it came to separating the flowers to the 3 species, the median clustering method did the best, while the single method did the worst in this regards. 
...
This interesting dataset offers two interesting items:
...
This way we can create an hierarchical clustering on the 306 genes expression values on the train and the test data and compare the two to see how stable are the results.
...
We can see that the top most (small) cluster is somewhat preserved between the two trees. However, a large spageti-like tangle of lines is indicating the the two trees are far from beeing identical.
...
To conclude: we see that the clustering alogirthm resulted in trees which are singnificantly similar in both the training and the test datasets beyond chance, but that this similarity is ristricted to only a very small proportion of genes.
...
This data can be visualized using a (costumed made) parallel coordinates plot:
...
For this data, it can be quite helpful to see a heatmap of the votes across the years. The ordering of the rows is tricky. First, the distance of the vectors (later used for the clustering) should be done after transformation (since we are dealing with proportion of votes). In this case I used the arcsin transformation (a logit transformation could also work, but the arcsin is safer for dealing with 0/1 observations). But given the clusters, we wish to order the leaves (as much as possible), so to take into account the missing value clusterings. So we in fact have two clusters, one for the raw values, and another for the "shadow matrix" (i.e.: the matrix with 0/1, indicating if a value was missing or not).
...
We see that the two clustering give similar results for: "Alabama", "Georgia", "Louisiana", "Arkansas", "Florida", "Texas", "South Carolina", "Mississippi".

There are also several other sub-trees which are identical between the two methods. The biggest difference lies in several "rouge" states which are placed differently in the two clustering algorithms. They are: Vermont, Michigan, Maine, Hawaii, New Jersey, West Virginia, and Oklahoma.
...
2. The warm-blooded vertebrates, which live in groups, have hair, can't fly, and mostly aren't endangered.
3. The cold-blooded vertebrates, without hair, can't fly, and aren't endangered.
...
We see that the the different methods (other than ward.D and ward.D2), all give quite different results. So would the above analysis be different if we had used another clustering algorithm?

For this purpose we compare the clustering solution of each algorithm with one another, when cut to k=4 clusters, using the Fowlkes-Mallows Index. This measure is similar to rand (or rand adjusted) index, and gives a value of 1 when the two clusters confirm, and 0 when they do not:
...
We removed the "median" method since it did not have k=4 possible. In general, the results seems sensitive the the algorithm used, and the different algorithm methods do not seem to agree with one another (with regards to k=4), so further analyses may be in place in order to decide on which algorithm and interpretation are most appropriate for this data.



(Other possible datasets for the future: chorSub, flower, plantTraits, pluton, ruspini, agriculture)