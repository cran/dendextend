...
#    are_they_equal <- NULL
   ...
   if(!(is.dendrogram(current))) return("current is not a dendrogram")
   ...
   if(use.tip.label.order) { 
      ...
      if(!is.logical(result)) return(paste("Difference in labels order - ", result, collapse = ""))
   }
   
   if(use.tip.label) { 
#       all.equal(c("a","b"),c("b","a"))
      result <- all.equal(sort(labels(target)), sort(labels(current)))   
      if(!is.logical(result)) return(paste("Difference in labels - ", result, collapse = ""))
   }   
   ...
   if(use.topology) {
      ...
      if(!(is.null(target_de) & is.null(current_de))) {
         return(paste("Dendrograms contain diffreent edges (i.e.: topology). Unique edges in target:", 
                      "|", 
